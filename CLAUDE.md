...
## XML-Driven Image Conversion System (2025-11-23)

### Overview
**Goal:** Replace programmatic test images with actual PNG files loaded from flash memory.
**Approach:** Create XML-based configuration system for converting multiple PNG images to C headers.

### Implementation

#### Phase 1: Central Configuration Header
**File Created:** `main/main.h`

**Purpose:** Single source of truth for screen/image format configuration
- Defines `SCREEN_FORMAT_RGB888` or `SCREEN_FORMAT_RGB565`
- All graphics (screen + images) use the same format
- Includes validation to prevent multiple formats being defined
- Parsed by both conversion script and C code

**Current Setting:** RGB888 (24-bit) mode enabled

#### Phase 2: XML Manifest
**File Created:** `images.xml` (project root)

**Structure:**
```xml
<images>
    <image>
        <source>tanmatsulogo.png</source>
        <target>main/logo_image.h</target>
        <name>logo</name>
    </image>
    <!-- + 3 test images -->
</images>
...
**Features:**
- Declarative configuration of all images to convert
- Source PNG → Target C header mapping
- Base name for generated constants (WIDTH, HEIGHT, _image_data)

#### Phase 3: Conversion Script Rewrite
**File Modified:** `convert_logo.py`

**Complete Rewrite:**
- Parses `main/main.h` to detect RGB565 vs RGB888 format
- Parses `images.xml` for list of images to convert
- Processes all images in a single run
- Supports two output formats:
  - **RGB565:** 16-bit (2 bytes/pixel) - `uint16_t` array
  - **RGB888:** 24-bit packed (3 bytes/pixel) - `uint8_t` array with **BGR byte order**

**Key Features:**
- No downscaling (PNG dimensions used as-is)
- Flattens transparency onto black background
- Auto-generated headers with dimension defines
- Readable hex formatting (12 values/row for RGB565, 24 bytes/row for RGB888)

**BGR Byte Order Fix:**
The PAX library/display expects BGR (Blue, Green, Red) byte order, not RGB:
- Line 91: `pixel_data.extend([b, g, r])` (not [r, g, b])
- Initially had red/blue channels swapped
- Fixed by reversing color order in conversion

#### Phase 4: Main Code Integration
**File Modified:** `main/main.c`

**Changes:**
1. Added `#include "main.h"` for format detection
2. Added includes for test image headers:
   - `test_square_image.h`
   - `test_horizontal_image.h`
   - `test_vertical_image.h`
3. Removed programmatic test buffer creation (lines 315-328)
4. Replaced with conditional image loading:
   ```c
   #ifdef SCREEN_FORMAT_RGB888
       pax_buf_init(&test_square, (void*)test_square_image_data,
                    TEST_SQUARE_WIDTH, TEST_SQUARE_HEIGHT, PAX_BUF_24_888RGB);
       // ... (same for wide/tall)
   #else
       // RGB565 versions
   #endif
   ```
5. Updated logo buffer init to use conditional format
6. Drawing code unchanged (PAX handles format automatically)

### Generated Files

**Conversion Output (RGB888 mode):**
- `main/logo_image.h` - 200×200 logo (117.2 KB)
- `main/test_square_image.h` - 200×200 test image (117.2 KB)
- `main/test_horizontal_image.h` - 200×50 test image (29.3 KB)
- `main/test_vertical_image.h` - 50×200 test image (29.3 KB)

**Total:** ~293 KB of image data in flash

**Header Format Example:**
```c
// Auto-generated image: test_square
// Source: test_square.png
// Size: 200×200, Format: RGB888 (3 bytes/pixel)
// Generated by convert_logo.py - DO NOT EDIT MANUALLY

#pragma once
#include <stdint.h>

#define TEST_SQUARE_WIDTH  200
#define TEST_SQUARE_HEIGHT 200

// RGB888 packed pixel data (3 bytes per pixel: B,G,R,B,G,R,... - BGR byte order)
static const uint8_t test_square_image_data[120000] = {
    0x03, 0xC3, 0xFF, 0x03, 0xC3, 0xFF, ...
};
```
...
**Build Status:** ✅ SUCCESS
- Binary size: 870 KB
- Format: RGB888 (24-bit packed, BGR byte order)
- No compilation errors or warnings

### Usage Workflow

**For Developers:**
1. Set desired format in `main/main.h` (RGB888 or RGB565)
2. Add image entries to `images.xml`
3. Run `python3 convert_logo.py` to generate all headers
4. Run `make build` to compile

**Format Switching:**
To switch from RGB888 to RGB565:
1. Edit `main/main.h` - comment RGB888, uncomment RGB565
2. Run `python3 convert_logo.py` (regenerates all headers)
3. Run `make build`

All conditional compilation and format detection is automatic.

### Technical Specifications

**RGB888 Format:**
- **Storage:** Packed `uint8_t` array (3 bytes/pixel)
- **Byte Order:** BGR (Blue, Green, Red)
- **Memory:** 3× width × height bytes
- **PAX Buffer Type:** `PAX_BUF_24_888RGB`

**RGB565 Format:**
- **Storage:** `uint16_t` array (2 bytes/pixel)
- **Encoding:** 5-bit red, 6-bit green, 5-bit blue
- **Memory:** 2× width × height bytes
- **PAX Buffer Type:** `PAX_BUF_16_565RGB`

### Issues and Fixes

**Issue 1: Red/Blue Channel Swap**
- **Problem:** Test images displayed with red and blue colors swapped
- **Root Cause:** Conversion generated RGB byte order, but display expects BGR
- **Fix:** Changed `convert_logo.py` line 91 from `[r, g, b]` to `[b, g, r]`
- **Result:** ✅ Colors now display correctly

### Testing Status

**Hardware Testing:** ✅ COMPLETED
- PNG images load and display correctly
- Colors accurate (after BGR fix)
- No memory issues
- Logo and test images render as expected
- Image positioning updated by user for optimal layout

### Future Enhancements

Possible improvements:
1. Add image compression support (RLE, LZ4)
2. Support for alpha channel/transparency
3. Automatic palette generation for indexed color modes
4. Build system integration (auto-run conversion on PNG changes)
5. Support for additional formats (JPEG, BMP)
6. Image scaling/cropping options in XML
